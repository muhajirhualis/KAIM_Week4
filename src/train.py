...
def load_data():
    ...
    data_path = os.path.join(script_dir, "..", "data", "processed", "modeling_dataset.csv")
...
def main():
    ...
    lr_metrics = train_and_log(lr_base, "LogisticRegression", X_train, X_test, y_train, y_test, tuned=False)
    rf_metrics = train_and_log(rf_base, "RandomForest", X_train, X_test, y_train, y_test, tuned=False)
    ...
    lr_params = {
        'C': [0.01, 0.1, 1, 10],
        'penalty': ['l2'],               # safer default (l1 + liblinear = deprecated combo)
        'solver': ['lbfgs', 'liblinear'],  # lbfgs supports l2, better for most cases
        'max_iter': [1000]
    }
    lr_search = RandomizedSearchCV(lr_tuned, lr_params, n_iter=8, cv=3, scoring='roc_auc', random_state=42)
    lr_search.fit(X_train, y_train)
    lr_tuned_metrics = train_and_log(lr_search.best_estimator_, "LogisticRegression", X_train, X_test, y_train, y_test, tuned=True)
    results["lr_tuned"] = {"model": lr_search.best_estimator_, "auc": lr_tuned_metrics["roc_auc"]}
    ...
    rf_tuned_metrics = train_and_log(rf_search.best_estimator_, "RandomForest", X_train, X_test, y_train, y_test, tuned=True)
    results["rf_tuned"] = {"model": rf_search.best_estimator_, "auc": rf_tuned_metrics["roc_auc"]}
    ...
    # Save best model
    best_model_path = os.path.join(PROJECT_ROOT, "models", "credit_risk_best.pkl")
