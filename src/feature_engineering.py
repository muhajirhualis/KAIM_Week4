...
class CustomerAggregator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # SNAPSHOT_DATE = pd.Timestamp('2019-02-01')
        ...
        # Flatten columns
        agg_df.columns = [
            '_'.join(str(x) for x in col) if isinstance(col, tuple) else col
        ...
        # agg_df['Recency'] = (SNAPSHOT_DATE - pd.to_datetime(agg_df['last_tx_time'])).dt.days
        # agg_df['Recency'] = agg_df['Recency'].clip(lower=0)

        agg_df['std_value'] = agg_df['std_value'].fillna(0)
        return agg_df


# 2. Categorical Encoder (One-Hot)
# ================================
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        for col in self.cat_cols:
            if col in X.columns:
                top_cats = X[col].value_counts().head(
                    self.top_n).index.tolist()
    ...
    def transform(self, X):
        ...
        for col in self.cat_cols:
            if col in df.columns:
                ...
                df[col +
                    '_enc'] = df[col].where(df[col].isin(self.top_cats_[col]), 'Other')
...
class WoETransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, woe_cols=None):
        self.woe_cols = woe_cols or [
            'avg_value', 'n_transactions', 'fraud_rate']
        self.woe_models_ = {}
...
def build_feature_pipeline(use_woe=False):
    ...
    # Step 3: Missing value handler
    miss = MissingValueHandler(strategy='median')

    # Step 4: Scaling — ONLY numeric columns (exclude CustomerId, one-hot cols)
    numeric_features = [
