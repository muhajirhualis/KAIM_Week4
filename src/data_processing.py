...
class CustomerAggregator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Handle std of single-transaction users
        ...
        # Fill NaN std (single tx → std = 0)
        for col in ['Amount_std', 'Value_std']:
...
def build_feature_pipeline(use_woe=False):
    """Build end-to-end pipeline: aggregate → extract → encode → scale."""
    steps = [
    ...
    # Final scaler for numeric features
    numeric_features = [
        'total_amount', 'avg_amount', 'std_amount', 'n_transactions',
    ...
    preprocessor = ColumnTransformer(
        ...
        remainder='passthrough',  # keep one-hot features unscaled
