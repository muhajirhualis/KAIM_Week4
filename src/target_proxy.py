...
class HighRiskLabeler(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, snapshot_date='2019-02-01', random_state=42):
        """
        Parameters
        ----------
    ...
    def _compute_rfm(self, df):
        # Use the SAME snapshot as in data_processing.py
        ...
        rfm = df.groupby('CustomerId').agg(
            Recency=('TransactionStartTime', lambda dates: (
                SNAPSHOT_DATE - dates.max()).days),
    ...
    def fit(self, X, y=None):
        ...
        rfm_scaled = self.scaler_.fit_transform(
        ...
        logger.info(
            f"✅ High-risk cluster identified: {self.high_risk_cluster_}")
        return self
    ...
    def transform(self, X):
        ...
        rfm = self._compute_rfm(X)
        rfm_scaled = self.scaler_.transform(
            rfm[['Recency', 'Frequency', 'Monetary']])
        clusters = self.kmeans_.predict(rfm_scaled)
        rfm['cluster'] = clusters
        rfm['is_high_risk'] = (
            rfm['cluster'] == self.high_risk_cluster_).astype(int)
